//Edit Equipment Menu View, where individual eq information is updated
...
Ext.define('AssetAndEquipmentSurvey.view.Task', {
    ...
    config: {
        title: LocaleManager.getLocalizedString('Edit Equipment',
         ...
         */
		 ...
		 //Rachel A. Sullivan 05.02.2018 Added description to fieldOrder
		 //Rachel A. Sullivan 05.07.2018 Added condition, option1, option2, property_type, and ai_status to fileOrder
		 
        fieldOrder: ['survey_id', 'site_id', 'bl_id', 'fl_id', 'rm_id', 'eq_id', 'description', 'num_serial', 'eq_std', 'property_type',
            'dv_id', 'dp_id', 'marked_for_deletion', 'status', 'condition', 'em_id', 'ai_status', 'option2', 'option1', 'date_manufactured', 'survey_comments', 'date_last_surveyed'],
	...
	//Rachel A. Sullivan 05.07.2018 Added condition, option1, option2, property_type, and ai_status to fieldset
                    {
                        xtype: 'commontextfield',
                        name: 'condition',
						id: 'condition'
	...
	//Rachel A. Sullivan 05.02.2018 Added description to fieldset
					{
                        xtype: 'commontextareafield',
                        name: 'description'
                    },
    ...
    initialize: function () {
        ...
        if (me.getIsCreateView()) {
            me.setTitle(LocaleManager.getLocalizedString('Add Equipment', 'AssetAndEquipmentSurvey.view.Task'));
        }
    ...
    generateFields: function () {
        ...
        if (formItems && formItems.length > 0) {
            ...
            formItems = Ext.Array.filter(formItems, function (item) {
				...
				if(item.statusField == 'In Service'){
					item.condition = ["Deployed", "Getty Center Storage", "Getty Villa Storage"];
				} else if(item.statusField == 'Out of Service'){
					item.condition = ["Getty Center Storage", "Getty Villa Storage", "Emergency Bins", "Vendor Repair"];
    ...
    setCustomFieldsLabels: function () {
        var surveyCommentField = this.down('commontextareafield[name=survey_comments]'),
            surveyCommentFieldLabel = LocaleManager.getLocalizedString('Inspection Comments', 'AssetAndEquipmentSurvey.view.Task');
